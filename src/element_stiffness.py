...
class element_stiffness():
    def __init__(self):
        pass
    def element_stiffness_staggered(self,elem,Points,Weights,C,disp,disp_old,stress,strain,H_n):
        """
        ...
        for j in range(0,num_Gauss**num_dim):
            ...
            K_uu = K_uu + np.matmul(np.matmul(np.transpose(Bmat),C),Bmat)*(((1-phi)**2)+k_const) \
                *Weights[j]*det_Jacobian
                
            K_phiphi = K_phiphi + (G_c*l_0*np.matmul(np.transpose(dNdX),dNdX) + \
                + ((G_c/l_0)+2*H)*np.matmul(np.transpose([N[0]]),[N[0]]))\
                *Weights[j]*det_Jacobian
            
            H_n = H
        return K_uu,K_phiphi,H_n
