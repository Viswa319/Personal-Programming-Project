...
for step in range(0,num_step):
    ...
    for elem in range(0,num_elem):
        ...
        index_phi = assemble.assembly_index_phi_monolithic(elem,num_dof_phi,num_tot_var_u,num_node_elem,elements)
    ...
    for iteration in range(0,max_iter):
        ...
        global_K,global_force_red,reaction_force = boundary_condition(num_dof_u,nodes_bc,fixed_dof,global_K,global_force,disp,disp_bc,tot_inc)
        start_solve_time = time.time()
        
        sp_global_K = csc_matrix(global_K)
        disp_solve = spsolve(sp_global_K,-global_force_red)
        ...
        if (iteration == max_iter - 1):
            print("Solution has achieved its maximum number of iterations and failed to converge.!!!")
