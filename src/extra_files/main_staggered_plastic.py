...
for step in range(num_step):
    ...
    get_stress = solve_stress_strain(num_Gauss_2D, num_stress, num_node_elem, num_dof_u, elements, disp, Points, nodes, C, strain_energy,strain_plas,alpha,hardening,sigma_y)
    ...
    stress_plot.append(stress[0,0,1])
    strain_plot.append(strain[0,0,1])
    if (step + 1) % 1000 == 0:
...
with open("tensile_plas_disp.json", "w") as opf:    
    json.dump(datadict, opf, indent=4)
with open("tensile_plas_phi.json", "w") as opf:    
    json.dump(datadict_2, opf, indent=4)   
plot_load_displacement(strain_plot, stress_plot)
